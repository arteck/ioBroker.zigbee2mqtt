...
class ImageController {
    ...
    constructor(adapter) {
        ...
        // PERFORMANCE/MEMORY: In-Memory-Cache für bereits erzeugte Base64-Icons.
        // Ohne diesen Cache würde bei JEDEM WS-Reconnect (Z2M sendet dann erneut
        // das komplette bridge/devices-Payload) für JEDES Gerät erneut
        // sharp(...).metadata()/.resize() aufgerufen – native libvips-Allokationen,
        // die bei häufigen Reconnects zu spürbarem RAM-Zuwachs des Adapterprozesses
        // führen können, da der native Speicher nicht sofort ans OS zurückgegeben wird.
    ...
    async getDeviceIcon(device) {
        ...
        // MEMORY-FIX: Bereits erzeugtes Icon (Base64) aus dem In-Memory-Cache verwenden.
        // Der Cache-Key basiert auf Modellname + Zielgröße, da beides das Ergebnis beeinflusst.
        const cacheKey = `${z2mModel}::${imageSize}`;
        ...
        if (!iconFound || !iconFileName) {
            ...
            // Fehlgeschlagene Downloads NICHT dauerhaft cachen – evtl. ist die Quelle beim
            // nächsten Reconnect wieder erreichbar.
            return '';
        }
        ...
        const dataUrl = `data:image/png;base64,${icon.file.toString('base64')}`;
        // MEMORY-FIX: Ergebnis für dieses Modell/diese Größe cachen, damit bei künftigen
        // WS-Reconnects (erneutes bridge/devices) kein weiterer sharp()-Aufruf nötig ist.
        this._iconCache.set(cacheKey, dataUrl);
        return dataUrl;
    }
    ...
    async downloadIcon(adapter, url, namespace) {
        try {
            ...
            if (parsedUrl.protocol !== 'https:' || parsedUrl.hostname !== 'www.zigbee2mqtt.io') {
                this.adapter.log.warn(`Refusing to download icon from untrusted URL: ${url}`);
                return false;
            }
