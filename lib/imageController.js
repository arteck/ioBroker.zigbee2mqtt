...
class ImageController {
    ...
    sanitizeModelIDForImageUrl(modelName) {
        let modelNameString = modelName.replace('/', '_');
        return modelNameString.replace(/\u0000/g, '');
    }

    sanitizeZ2MDeviceName(deviceName) {
        let deviceNameString = deviceName.replace(/:|\s|\//g, '-')
    ...
    getZ2mDeviceImage(device) {
        if (device && device.definition && device.definition.model) {
            let icoString =  `https://www.zigbee2mqtt.io/images/devices/${this.sanitizeZ2MDeviceName(device.definition.model)}.jpg`;
    ...
    getSlsDeviceImage(device) {
        if (device && device.model_id) {
            let icoString = `https://slsys.github.io/Gateway/devices/png/${this.sanitizeModelIDForImageUrl(device.model_id)}.png`;
            return icoString.replace(/\u0000/g, '');
        }
    }
