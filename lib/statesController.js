...
class StatesController {
        ...
        if (device) {
            ...
            if (!this.incStatsQueue.some((x) => x && x.topic === messageObj.topic)) {
                ...
                if (this.incStatsQueue.length < 500) {
                    this.incStatsQueue.push({ ...messageObj, _ttl: ttl });
                } else {
    ...
    async setDeviceStateSafely(messageObj, device) {
        // debugDevices ist ein kommaseparierter String → exakter Teilvergleich per Split
        const debugList = this.logCustomizations.debugDevices
            ? String(this.logCustomizations.debugDevices).split(',').map((s) => s.trim())
            : [];
