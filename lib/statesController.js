...
class StatesController {
    ...
    constructor(adapter, deviceCache, groupCache, logCustomizations, createCache) {
        ...
        this.incStatsQueue = [];
        // Max. Verweildauer einer Nachricht in der incStatsQueue, bevor sie verworfen wird.
        // Zeit- statt zählerbasiert, da neu angelernte Geräte während des Interviews mehrere
        // bridge/devices-Zyklen (und damit processQueue()-Aufrufe) benötigen können, bis das
        // Geräteobjekt/die States angelegt sind. Ein reiner Aufruf-Zähler hätte z.B. das
        // "available: true" der Retained-".../availability"-Nachricht vorzeitig verworfen,
        // wodurch der available-State neu angelernter Geräte dauerhaft auf "false" hängen blieb.
        this.QUEUE_MAX_AGE_MS = 5 * 60 * 1000; // 5 Minuten
        this.timeOutCache = {};
    ...
    async processDeviceMessage(messageObj) {
        ...
        if (device) {
            ...
            //
            // Wichtig: Die Verwerf-Zeit basiert auf einem ZEITSTEMPEL (_queuedAt), nicht auf
            // einem Aufruf-Zähler. Grund: Bei neu angelernten Geräten kann bridge/devices
            // während des Interviews mehrfach gesendet werden (und processQueue() wird auch
            // durch bridge/devices-Events ANDERER Geräte ausgelöst). Ein reiner Zähler
            // würde die z.B. per Retained-MQTT-Topic ".../availability" bereits eingetroffene
            // "available: true"-Nachricht des neuen Geräts vorzeitig verwerfen, bevor das
            // Interview abgeschlossen und das Geräteobjekt angelegt ist – Folge: das
            // "available"-State bleibt dauerhaft auf dem Default-Wert "false" stehen.
            const existingIdx = this.incStatsQueue.findIndex((x) => x && x.topic === messageObj.topic);
            if (existingIdx !== -1) {
                const queuedAt = this.incStatsQueue[existingIdx]._queuedAt || Date.now();
                const age = Date.now() - queuedAt;
                if (age > this.QUEUE_MAX_AGE_MS) {
                    this.adapter.log.warn(`incStatsQueue: dropping message for unknown device ${messageObj.topic} after ${Math.round(age / 1000)}s`);
                    this.incStatsQueue.splice(existingIdx, 1);
                    return;
                }
                this.incStatsQueue[existingIdx] = { ...messageObj, _queuedAt: queuedAt };
            } else {
                if (this.incStatsQueue.length < 500) {
                    this.incStatsQueue.push({ ...messageObj, _queuedAt: Date.now() });
                } else {
    ...
    async setDeviceStateSafely(messageObj, device) {
        ...
        const pushToQueue = (msg) => {
            ...
            if (existingIdx !== -1) {
                ...
                const age = Date.now() - queuedAt;
                if (age > this.QUEUE_MAX_AGE_MS) {
                    this.adapter.log.warn(`incStatsQueue: dropping message for ${msg.topic} after ${Math.round(age / 1000)}s (state not yet created)`);
                    this.incStatsQueue.splice(existingIdx, 1);
                    return;
                }
                this.incStatsQueue[existingIdx] = { ...msg, _queuedAt: queuedAt };
            } else if (this.incStatsQueue.length < 500) {
                this.incStatsQueue.push({ ...msg, _queuedAt: Date.now() });
            } else {
        ...
        const stateWrites = [];
        // Flag für available-Update (wenn last_seen vorhanden und config aktiv, oder generell
        // wenn echte Nutzdaten empfangen wurden – siehe Begründung weiter unten)
        let needSetAvailableTrue = false;

        // Bugfix: Wenn eine Nachricht ausschließlich das "available"-Property enthält
        // (z.B. aus dem ".../availability"-Topic oder bridge/event device_leave), soll
        // dessen Wert unverändert übernommen werden. Enthält die Nachricht jedoch
        // zusätzliche/echte Nutzdaten (state, brightness, temperature, ...), beweist das
        // bereits, dass das Gerät aktiv kommuniziert hat – unabhängig davon, ob Zigbee2MQTT
        // das (per Default deaktivierte) "availability"-Feature nutzt oder last_seen sendet
        // (advanced.last_seen ist bei Z2M ebenfalls per Default "disable"). Ohne diesen Fix
        // bleibt der "available"-State nach einem MQTT-/Adapter-Neustart dauerhaft auf
        // "false", obwohl die Geräte normal erreichbar und schaltbar sind.
        const payloadKeys = Object.keys(messageObj.payload);
        const hasOnlyAvailableKey = payloadKeys.length > 0 && payloadKeys.every((k) => k === 'available');
        if (!hasOnlyAvailableKey && payloadKeys.length > 0 && !device.ieee_address.includes('group')) {
