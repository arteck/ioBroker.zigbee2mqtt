...
class StatesController {
    ...
    async setDeviceStateSafely(messageObj, device) {
        ...
        // illuminance aus illuminance_raw berechnen, falls nicht bereits im Payload vorhanden
        if (
            messageObj.payload.illuminance_raw !== undefined &&
            messageObj.payload.illuminance_raw !== null &&
            messageObj.payload.illuminance === undefined
        ) {
            try {
                const rawVal = Number(messageObj.payload.illuminance_raw);
                if (!isNaN(rawVal)) {
                    // Zigbee-Formel: illuminance (lux) = 10 ^ ((raw - 1) / 10000)
                    const luxVal = Math.round(Math.pow(10, (rawVal - 1) / 10000));
                    const fullPath = `${device.ieee_address}.additional`;

                    await this.adapter.setObjectNotExistsAsync(fullPath, {
                        type: 'channel',
                        common: { name: 'hidden channelstate' },
                        native: {},
                    });
                    await this.adapter.setObjectNotExistsAsync(`${fullPath}.illuminance`, {
                        type: 'state',
                        common: {
                            name: 'illuminance',
                    ...
                    await this.adapter.setStateChangedAsync(`${fullPath}.illuminance`, luxVal, true);
                    this.adapter.log.debug(`illuminance_raw=${rawVal} → illuminance=${luxVal} lux (${device.ieee_address})`);
                }
            } catch (e) {
                this.adapter.log.warn(`Cannot calculate illuminance for ${device.ieee_address}: ${e}`);
            }
        }
