...
class DeviceController extends dmUtils.DeviceManagement {
    ...
    async getDeviceDetails(id, _action, _context) {
        ...
        // last_seen aus ioBroker-State lesen
        let lastSeenText = '—';
        try {
            const lastSeenState = await this.adapter.getStateAsync(`${device.ieee_address}.last_seen`);
            if (lastSeenState && lastSeenState.val != null && lastSeenState.val !== '') {
                lastSeenText = String(lastSeenState.val);
            }
        } catch (_e) {
            // State nicht vorhanden – ignorieren
        }

        // link_quality aus ioBroker-State lesen
        let linkQualityText = '—';
        try {
            const lqState = await this.adapter.getStateAsync(`${device.ieee_address}.link_quality`);
            if (lqState && lqState.val != null) {
                linkQualityText = `${lqState.val} / 255`;
            }
        } catch (_e) {
            // State nicht vorhanden – ignorieren
        }
        ...
        const tabs = {
            _tab_info: {
                ...
                label: 'Info',
                items: {
                    _header_device: { type: 'header', text: displayName, size: 4, newLine: true },
                    _div1: { type: 'divider', color: 'primary' },
                    ieee_address:     { type: 'staticInfo', label: 'IEEE',          data: device.ieee_address,            addColon: true, highlight: true, newLine: true },
                    friendly_name:    { type: 'staticInfo', label: 'Friendly Name', data: device.id,                      addColon: true, highlight: true, newLine: true },
                    manufacturer:     { type: 'staticInfo', label: 'Manufacturer',  data: device.manufacturer || '—',     addColon: true, highlight: true, newLine: true },
                    model:            { type: 'staticInfo', label: 'Model',         data: device.model || '—',            addColon: true, highlight: true, newLine: true },
                    modelDescription: { type: 'staticInfo', label: 'Description',   data: device.modelDescription || '—', addColon: true, highlight: true, newLine: true },
                    powerSource:      { type: 'staticInfo', label: 'Power Source',  data: device.powerSource || '—',      addColon: true, highlight: true, newLine: true },
                    _div2: { type: 'divider', color: 'primary' },
                    linkQuality: { type: 'staticInfo', label: 'Link Quality', data: linkQualityText, addColon: true, highlight: true, newLine: true },
                    lastSeen:    { type: 'staticInfo', label: 'Last seen',    data: lastSeenText,    addColon: true, highlight: true, newLine: true },
                },
            },
                    _tab_tech: {
                        type: 'panel',
        ...
        // Detail-Tab für States mit langen Labels
        if (hasDetails) {
        ...
        // Schedule-Tab nur für Thermostate mit Schedule-States
        if (hasSchedule) {
            tabs._tab_schedule = {
                type: 'panel',
                label: 'Schedule',
                items: {
                    ...
                    _div_schedule: { type: 'divider', color: 'primary' },
                    ...scheduleItems,
                },
            };
        }
