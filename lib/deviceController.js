...
class DeviceController {
    ...
    async _createOrUpdateSingleDevice(device, isGroup) {
        ...
        // Veraltete scene_*-States bereinigen – getObjectsAsync statt getStatesAsync,
        // damit auch Objekte ohne gesetzten Wert erfasst werden
        const sceneObjects = await this.adapter.getObjectsAsync(`${device.ieee_address}.scene_*`);
        if (sceneObjects) {
            for (const sceneObjId of Object.keys(sceneObjects)) {
                ...
                if (!device.states.find((x) => x.id === stateID)) {
                    await this.adapter.delObjectAsync(sceneObjId);
                }
            }
        }
