...
function createFromExposes(deviceID, ieee_address, definitions) {
	const states = [];
...
function applyExposes(devices, deviceID, ieee_address, definitions) {
	// create or update device from exposes
	//for (const definition of definitions) {
	// check if device is mapped
	const existsDevice = devices.find(x => x.id == deviceID);

	if (definitions.hasOwnProperty('exposes')) {
		try {
			const newDevice = createFromExposes(deviceID, ieee_address, definitions);
			if (!existsDevice) {
				devices.push(newDevice);
			} else {
				existsDevice.states = newDevice.states;
			}
		} catch (e) {
			//console.log(`Wrong expose devicedefinition ${definitions.vendor} ${definitions.model}`);
		}
