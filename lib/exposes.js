                ...
                switch (expose.name) {
                    case 'action': {
                        ...
                        for (const actionName of expose.values) {
                            ...
                            else if (actionName.includes('color_temp')) {
                                pushToStates({
                                    id: 'colortemp',
                                    prop: 'action',
                                    ...
                                    unit: config.useKelvin == true ? 'K' : 'mired',
                                    getter: (payload) => {
                                        if (!payload.action.included('color_temp')) {
                                            return undefined;
                                        }

                                        let result = undefined;
                                        if (payload.action_color_temperature) {
                                            result = payload.action_color_temperature;
                                        }
                                        else if (payload.color_temperature) {
                                            result = payload.color_temperature;
                                        }
                                        else if (payload.color_temp) {
                                            result = payload.color_temp;
                                        }

                                        if (config.useKelvin == true && result) {
                                            return utils.miredKelvinConversion(result);
                                        }
                                        else {
                                            return result;
                                        }
                            ...
                            else if (actionName.includes('color_wheel')) {
                                pushToStates(statesDefs.color_read, expose.access);
                            }
                            else {
                                pushToStates({
                                    id: actionName.replace(/\*/g, ''),
