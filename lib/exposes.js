...
function createDeviceFromExposes(devicesMessag, config, adapter) {
    const states = [];
    let scenes = [];
            ...
            case 'enum':
                switch (expose.name) {
                    case 'action': {
                        ...
                        // Hack for DIYRuZ Device
                        const wildcardValues = expose.values.filter(x => x.startsWith('*'));
                        adapter.log.warn(JSON.stringify(wildcardValues));
                        if (wildcardValues && wildcardValues.length > 0) {
                            for (const endpointName of [...new Set(definition.exposes.filter(x => x.endpoint).map(x => x.endpoint))]) {
                                adapter.log.warn(JSON.stringify(endpointName));
